...
pipeline {
    ...
    stages {
        ...
        stage('Clone Repository') {
            steps {
                script {
                    clone("https://github.com/Umanagalla27/tws-e-commerce-app_hackathon.git","master")
                }
            }
        }
                ...
                script {
                    withCredentials([usernamePassword(credentialsId: 'github-credentials', usernameVariable: 'GIT_USERNAME', passwordVariable: 'GIT_PASSWORD')]) {
                        sh """
                          ...
                          sed -i "s|image: .*|image: ${DOCKER_IMAGE_NAME}:${DOCKER_IMAGE_TAG}|g" kubernetes/08-easyshop-deployment.yaml
                          if [ -f kubernetes/12-migration-job.yaml ]; then
                            sed -i "s|image: .*|image: ${DOCKER_MIGRATION_IMAGE_NAME}:${DOCKER_IMAGE_TAG}|g" kubernetes/12-migration-job.yaml
                          fi
                          ...
                          git remote set-url origin "https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/Umanagalla27/tws-e-commerce-app_hackathon.git"
